  ...
  return (
    ...
    bg-slate-950 sm:flex-wrap'>
        ...
        sm:items-center sm:flex-wrap gap-x-4'>
            ...
            sm:text-center'>{exercise.name.replaceAll("_"," ")}
            </h2>
            <p className='text-sm text-slate-400 capitalize'>{exercise.type}</p>
        </div>
        <div className='flex flex-col'>
            <h3 className='text-slate-400'>Muscle Groups</h3>
            <p className='capitalize'>{exercise.muscles.join(' & ')}</p>
        </div>

        <div className='flex flex-col bg-slate-950 rounded
        gap-2 '>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2'>
            {['reps', 'rest', 'tempo'].map(info => {
                return(
                    ...
                    border-solid border-slate-500 w-full'>
                    <h3 className='capitalize text-slate-400 text-sm'>{info 
                        === 'reps'? `${exercise.unit}`:info}</h3>
            ...
            <h3 className='text-slate-400 text-sm capitalize'>Sets completed
            </h3>
