  ...
  return(
    <div className='flex flex-col gap-4'>
      <div className='flex items-center justify-center gap-2'>
        <p className='test-3xl sm:text-4xl md:text-5xl
  ...
  return (
    ...
    {['It\'s', 'Huge','o\'clock']}>
      ...
      {Object.keys(WORKOUTS).map((type, typeIndex) => {
        return(
          <button onClick={() => {
           ...
           duration-200 px-4 hover:border-blue-600 py-3 
           rounded-lg ${type===poison ? 
      ...
      border-blue-400 rounded-lg flex flex-col'>
          <button onClick={toggleModal} className='relative p-3 flex items-center 
          justify-center'>
            <p className='capitalize'>{muscles.length == 0 ?'Select muscle groups':
              muscles.join('')}</p>
            <i className="fa-solid absolute right-3 
            top-1/2 -translate-y1/2 fa-caret-down"></i>
          </button>
          {showModal && (
